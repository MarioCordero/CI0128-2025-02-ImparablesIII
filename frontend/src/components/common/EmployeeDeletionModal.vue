  ...
  >
    ...
    >
      ...
      <div class="text-gray-600 text-center mb-6 leading-relaxed">
        ...
        <p v-if="payrollCount > 0" class="text-sm text-blue-600 font-medium">
          Este empleado aparece en {{ payrollCount }} planilla(s) histórica(s) que se preservarán.
        </p>
      </div>
      ...
      <div v-if="firstConfirmed && !secondConfirmed" class="mb-6">
        ...
        <textarea
          ...
          rows="3"
          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
          placeholder="Ingrese el motivo de la baja del empleado"
        ></textarea>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="firstConfirmed && !secondConfirmed" class="text-center">
          <p class="text-sm text-gray-500 mb-3">
            Paso 2 de 2: Esta acción no se puede deshacer automáticamente. ¿Confirmar definitivamente?
          </p>
          <div class="flex gap-3 justify-center">
            <button
              @click="confirmSecond"
...
<script>
export default {
  name: 'EmployeeDeletionModal',
  props: {
    title: {
      type: String,
      default: 'Eliminar Empleado'
    },
    message: {
      type: String,
      required: true
